...
// this exercise asks to test letters, punctuation, and spaces
func getRandomUnicodeCharacter(rng *rand.Rand) rune {
	valid := false
	var r rune
	for !valid {
		r = rune(rng.Intn(0x1000))
		valid = unicode.IsLetter(r) || unicode.IsSpace(r) || unicode.IsPunct(r)
	}
	return r
}
...
func randomPalindrome(rng *rand.Rand) string {
	...
	runes := make([]rune, n)
	for i := 0; i < (n+1)/2; i++ {
		r := getRandomUnicodeCharacter(rng)
		runes[i] = r
		runes[n-1-i] = r
	}
	return string(runes)
